...
export const imageTagRegExp = new RegExp(
	"^[a-zA-Z0-9.-_/]{1,256}:(?![.-])[a-zA-Z0-9.-_]{1,128}$",
);

// GLOB Patterns
export const FROM_DIRECTIVE_PATTERN =
	/^\s*FROM\s*([\w-/:]*)(\s*AS\s*[a-z][a-z0-9-_\\.]*)?$/i;

export const COMPOSE_FILE_GLOB_PATTERN =
	"**/*[cC][oO][mM][pP][oO][sS][eE]*.{[yY][aA][mM][lL],[yY][mM][lL]}";
