...
} from "./ScaffoldingWizardContext";

export type OverwritePolicy = "overwrite" | "ask" | "skip";

export class ScaffoldFileStep<
	TWizardContext extends ScaffoldingWizardContext,
> extends AzureWizardExecuteStep<TWizardContext> {
	public constructor(
		private readonly fileType: ScaffoldedFileType,
		private readonly overwritePolicy: OverwritePolicy,
	...
	public async execute(
		wizardContext: TWizardContext,
		progress: vscode.Progress<{ message?: string; increment?: number }>,
	): Promise<void> {
		progress.report({
			message: vscode.l10n.t("Creating '{0}'...", this.fileType),
		});
		...
		if (this.overwritePolicy === "ask") {
			await this.promptForOverwriteIfNeeded(
				wizardContext,
				output,
				outputPath,
			);
		} else if (
			this.overwritePolicy === "skip" &&
	...
	private async getInputPath(wizardContext: TWizardContext): Promise<string> {
		...
		return (
			(settingsTemplatesPath &&
				(await this.scanUpwardForFile(
					path.join(settingsTemplatesPath, subPath),
				))) ||
			(await this.scanUpwardForFile(
				path.join(defaultTemplatesPath, subPath),
			))
		);
	}

	private async scanUpwardForFile(
		file: string,
		maxDepth: number = 1,
	): Promise<string> {
		...
		for (let i = 0; i <= maxDepth; i++) {
			...
			const parentDir = path.resolve(
				path.join(path.dirname(currentFile), ".."),
			);
	...
	private async promptForOverwriteIfNeeded(
		wizardContext: TWizardContext,
		output: string,
		outputPath: string,
	): Promise<void> {
		...
		const response = await wizardContext.ui.showWarningMessage(
			prompt,
			overwriteAll,
			overwrite,
