...
export class NodeGatherInformationStep extends GatherInformationStep<NodeScaffoldingWizardContext> {
	...
	public async prompt(
		wizardContext: NodeScaffoldingWizardContext,
	): Promise<void> {
		...
		if (nodePackage.scripts?.start) {
			...
			wizardContext.nodeDebugCmdParts = [
				...
				nodePackage.main || main || "index.js",
			];
		...
		if (!wizardContext.dockerBuildContext) {
			...
			wizardContext.dockerBuildContext = path.dirname(
	...
	protected setTelemetry(wizardContext: NodeScaffoldingWizardContext): void {
		wizardContext.telemetry.properties.packageHasStartScript =
			this.packageHasStartScript.toString();
	}
}
