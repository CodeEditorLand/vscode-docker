...
export class ChoosePlatformStep extends TelemetryPromptStep<ScaffoldingWizardContext> {
	...
	public async prompt(
		...
		const platforms =
			this.platformsList || (AllPlatforms as readonly Platform[]);
	...
	public async getSubWizard(
		wizardContext: ScaffoldingWizardContext,
	): Promise<IWizardOptions<ScaffoldingWizardContext> | undefined> {
		...
		switch (wizardContext.platform) {
			...
			case "Other":
				// Too simple to justify having their own methods
				return {
					promptSteps: [new GatherInformationStep()],
				};
	...
	protected setTelemetry(wizardContext: ScaffoldingWizardContext): void {
		wizardContext.telemetry.properties.configurePlatform =
