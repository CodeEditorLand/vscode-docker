...
export class NetSdkChooseBuildStep extends TelemetryPromptStep<NetChooseBuildTypeContext> {
	public async prompt(
		wizardContext: NetChooseBuildTypeContext,
	): Promise<void> {
		await this.ensureCSharpExtension(wizardContext);

		// get workspace momento storage
		const containerBuildOption =
			await ext.context.workspaceState.get<NetContainerBuildOption>(
		...
		const opt: vscode.QuickPickOptions = {
			...
			placeHolder: vscode.l10n.t(
				"How would you like to build your container image?",
			),
		};
		...
		await ext.context.workspaceState.update(
			NetContainerBuildOptionsKey,
			wizardContext.containerBuildOption,
		);
	}
	...
	protected setTelemetry(wizardContext: NetChooseBuildTypeContext): void {
		wizardContext.telemetry.properties.netSdkBuildStep =
			wizardContext.containerBuildOption;
	}

	private async ensureCSharpExtension(
		wizardContext: NetChooseBuildTypeContext,
	): Promise<void> {
		try {
