...
export class PythonGatherInformationStep extends GatherInformationStep<PythonScaffoldingWizardContext> {
	...
	public shouldPrompt(
		...
		return (
			...
			!wizardContext.pythonProjectType
		);
	}

	private async getDjangoCmdParts(
		wizardContext: PythonScaffoldingWizardContext,
	): Promise<void> {
	...
	private async getFastAPICmdParts(
		wizardContext: PythonScaffoldingWizardContext,
	): Promise<void> {
	...
	private async getFlaskCmdParts(
		wizardContext: PythonScaffoldingWizardContext,
	): Promise<void> {
		const { args, bindPort } = this.getCommonProps(wizardContext);
	...
	private async getGeneralCmdParts(
		wizardContext: PythonScaffoldingWizardContext,
	): Promise<void> {
		const { app, args } = this.getCommonProps(wizardContext);

		wizardContext.pythonCmdParts = ["python", ...app, ...args];
	...
	private getCommonProps(wizardContext: PythonScaffoldingWizardContext): {
		...
		return {
			app:
				"module" in wizardContext.pythonArtifact
			...
			bindPort: wizardContext.ports
				? wizardContext.ports[0]
				: PythonDefaultPorts.get(wizardContext.pythonProjectType),
		};
	}
}
