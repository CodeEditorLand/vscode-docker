...
const djangoExcludePattern =
	"{.[Vv][Ee][Nn][Vv]/**,**/[AaWw][Ss][Gg][Ii].[Pp][Yy]}";
...
export class ChoosePythonArtifactStep extends ChooseArtifactStep<PythonScaffoldingWizardContext> {
	...
	public async prompt(
		wizardContext: PythonScaffoldingWizardContext,
	): Promise<void> {
		const items =
			(await resolveFilesOfPattern(
				wizardContext.workspaceFolder,
				this.globPatterns,
				wizardContext.platform === "Python: Django"
					? djangoExcludePattern
					: excludePattern,
			)) ?? [];

		const pickChoices: IAzureQuickPickItem<Item | undefined>[] = items.map(
			(i) => {
				return {
		...
		if (result === enterModuleChoice) {
			...
			const module = await wizardContext.ui.showInputBox({
				prompt: l10n.t(
					"Enter a Python module name (e.g. myapp.manage)",
				),
				validateInput: (value: string): string | undefined => {
					...
					return l10n.t(
						"Enter a valid Python module name (e.g. myapp.manage)",
					);
				},
			});
	...
	public shouldPrompt(
		...
		return (
			super.shouldPrompt(wizardContext) || !wizardContext.pythonArtifact
		);
	}
	...
	protected setTelemetry(
		...
		wizardContext.telemetry.properties.pythonArtifact =
			"module" in wizardContext.pythonArtifact ? "module" : "file";
	}
}
