...
export async function getMinimumCSharpExtensionExports(): Promise<CSharpExtensionExports> {
	...
	if (!cSharpExtension || !cSharpExtensionVersion) {
		throw new Error(
			vscode.l10n.t(
				"Cannot generate Dockerfiles for a .NET project unless the C# extension is installed.",
			),
		);
	} else if (semver.lt(cSharpExtensionVersion, minCSharpVersionString)) {
		throw new Error(
			vscode.l10n.t(
				"Cannot generate Dockerfiles for a .NET project unless version {0} or higher of the C# extension is installed.",
				minCSharpVersionString,
			),
		);
	}
