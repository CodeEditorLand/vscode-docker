...
export class NetCoreTryGetRandomPortStep extends GatherInformationStep<NetCoreScaffoldingWizardContext> {
	public async prompt(
		...
		try {
			wizardContext.suggestedRandomPorts =
				await this.tryGetPortFromLaunchSettings(
	...
	public shouldPrompt(
		...
		return (
			!wizardContext.suggestedRandomPorts?.length &&
			!!wizardContext.workspaceFolder &&
	 ...
	 */
	private async tryGetPortFromLaunchSettings(
		...
		for (const launchSettingsFile of launchSettingsFiles) {
			const launchSettingsJson = (await fse.readJSON(
				launchSettingsFile.fsPath,
