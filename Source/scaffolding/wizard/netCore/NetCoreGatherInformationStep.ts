...
import {
	...
	cSharpExtensionId,
	getMinimumCSharpExtensionExports,
} from "./netCoreStepUtils";
...
export class NetCoreGatherInformationStep extends GatherInformationStep<NetCoreScaffoldingWizardContext> {
	...
	public async prompt(
		...
		const projectInfo = await getNetCoreProjectInfo(
			...
			wizardContext.artifact,
		);

		if (projectInfo.length < 2) {
			throw new Error(
				vscode.l10n.t(
					"Unable to determine project info for '{0}'",
					wizardContext.artifact,
				),
			);
		}
		...
		if (
			...
			const regexMatch = /net(coreapp)?([\d.]+)/i.exec(
			...
			if (!regexMatch || regexMatch.length < 3) {
				throw new Error(
					vscode.l10n.t(
						"Unable to determine .NET target framework version for '{0}'",
						wizardContext.artifact,
					),
				);
			}
		...
		if (!wizardContext.serviceName) {
			wizardContext.serviceName = getValidImageNameFromPath(
				wizardContext.artifact,
			);
		}
		...
		if (!wizardContext.dockerfileDirectory) {
			...
			wizardContext.dockerfileDirectory = path.dirname(
				wizardContext.artifact,
			);
		}
	...
	public shouldPrompt(
		...
		return (
			!wizardContext.netCoreAssemblyName ||
			!wizardContext.netCoreRuntimeBaseImage ||
			!wizardContext.netCoreSdkBaseImage ||
	...
	protected setTelemetry(
		wizardContext: NetCoreScaffoldingWizardContext,
	): void {
		wizardContext.telemetry.properties.netCoreVersion =
			this.targetFramework;
	}

	private async ensureNetCoreBuildTasks(
		wizardContext: NetCoreScaffoldingWizardContext,
	): Promise<void> {
		...
		try {
			...
			await cSharpPromptConfig.update(
				cSharpPromptSetting,
				true,
			...
			await cSharpPromptConfig.update(
				cSharpPromptSetting,
				oldSuppressSettings.globalValue,
				vscode.ConfigurationTarget.Global,
			);
		}
	}
