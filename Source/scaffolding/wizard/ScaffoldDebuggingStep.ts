...
export class ScaffoldDebuggingStep extends AzureWizardExecuteStep<ScaffoldingWizardContext> {
	...
	public async execute(
		...
		progress.report({
			message: l10n.t("Adding debug configuration and tasks..."),
		});
		...
		switch (wizardContext.platform) {
			...
			case ".NET: Console":
				...
				await dockerDebugScaffoldingProvider.initializeNetCoreForDebugging(
					scaffoldContext,
					{
						...
						platformOS: (
							wizardContext as NetCoreScaffoldingWizardContext
						).netCorePlatformOS,
					},
				);
				break;
			...
			case "Python: General":
				...
				await dockerDebugScaffoldingProvider.initializePythonForDebugging(
					...
					{
						projectType: (
							wizardContext as PythonScaffoldingWizardContext
						).pythonProjectType,
						target: (
							wizardContext as PythonScaffoldingWizardContext
						).pythonArtifact,
			...
			default:
				throw new Error(
					l10n.t("Invalid platform for debug config scaffolding."),
				);
		}
	}
