...
export class ChoosePortsStep extends TelemetryPromptStep<ScaffoldingWizardContext> {
	...
	public async prompt(
		...
		const suggestedPorts = wizardContext.suggestedRandomPorts?.length
		...
		wizardContext.ports = splitPorts(
			await wizardContext.ui.showInputBox(opt),
		);
	}
	...
	protected setTelemetry(wizardContext: ScaffoldingWizardContext): void {
		wizardContext.telemetry.measurements.numPorts =
			wizardContext.ports?.length ?? 0;
	}
}
