...
async function verifyIsRunningInWorkspace(
	...
	if (!!config.get("showRemoteWorkspaceWarning")) {
		...
		if (remoteInfo.extensionKind === DockerExtensionKind.ui) {
			...
			await callWithTelemetryAndErrorHandling(
				"verifyIsWorkspaceExtension",
				(newContext: IActionContext) => {
...
function updateExtensionKind(newKind: string): void {
	...
	if (
		typeof values.workspaceValue === "object" &&
		values.workspaceValue !== null &&
