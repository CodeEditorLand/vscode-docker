...
export async function runImageInteractive(
	context: IActionContext,
	node?: ImageTreeItem,
): Promise<void> {
...
async function runImageCore(
		...
		node = await ext.imagesTree.showTreeItemPicker<ImageTreeItem>(
			ImageTreeItem.contextValue,
			{
				...
				noItemFoundErrorMessage: l10n.t(
					"No images are available to run",
				),
			},
		);
	}
	...
	context.telemetry.properties.containerOS =
		inspectResult?.[0]?.operatingSystem || "linux";
