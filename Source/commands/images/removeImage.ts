...
export async function removeImage(
	...
	nodes = await multiSelectNodes(
		{
			...
			noItemFoundErrorMessage: vscode.l10n.t(
				"No images are available to remove",
			),
		},
		ext.imagesTree,
	...
	if (nodes.length === 1) {
		confirmRemove = vscode.l10n.t(
			'Are you sure you want to remove image "{0}"? If there are other tags or child images for this image, only the tag will be removed.',
			nodes[0].fullTag,
		);
	} else {
		confirmRemove = vscode.l10n.t(
			"Are you sure you want to remove the selected images? If there are other tags or child images for these images, only the tag will be removed.",
		);
	}
