...
export class GetRegistryTargetPromptStep extends AzureWizardPromptStep<PushImageWizardContext> {
	public async configureBeforePrompt(
		wizardContext: PushImageWizardContext,
	): Promise<void> {
		...
		if (this.registryIsDeterminate(wizardContext.initialTag)) {
			wizardContext.connectedRegistry =
				await this.tryGetConnectedRegistryForPath(
					wizardContext,
					wizardContext.initialTag,
				);
		}
	}
	...
	public async prompt(wizardContext: PushImageWizardContext): Promise<void> {
		try {
			const pickedNode = await registryExperience<
				CommonRegistry | AzureSubscriptionRegistryItem
			>(wizardContext, {
				contextValueFilter: {
			...
			if (isAzureSubscriptionRegistryItem(pickedNode.wrappedItem)) {
				...
				wizardContext.azureSubscriptionNode =
					pickedNode as UnifiedRegistryItem<AzureSubscriptionRegistryItem>;
			} else {
				wizardContext.connectedRegistry =
					pickedNode as UnifiedRegistryItem<CommonRegistry>;
			}
		} catch (error) {
	...
	public shouldPrompt(wizardContext: PushImageWizardContext): boolean {
		return (
			!wizardContext.connectedRegistry &&
			!this.registryIsDeterminate(wizardContext.initialTag) &&
			this.shouldPromptForRegistry
		);
	}

	private async tryGetConnectedRegistryForPath(
		...
		const allRegistries = await vscode.window.withProgress(
			...
			() =>
				ext.registriesTree.getConnectedRegistries(
					normalizedImageNameInfo.normalizedRegistry,
				),
		);

		return allRegistries.find(
			(registry) =>
				registry.wrappedItem.baseUrl.authority ===
				normalizedImageNameInfo.normalizedRegistry,
		);
	}
	...
	private get shouldPromptForRegistry(): boolean {
		return vscode.workspace
			...
			.get("promptForRegistryWhenPushingImages", true);
	}
