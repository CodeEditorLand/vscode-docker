...
export class CreatePickAcrPromptStep extends AzureWizardPromptStep<PushImageWizardContext> {
	public async prompt(wizardContext: PushImageWizardContext): Promise<void> {
		...
		>[] = acrs.map(
			(acr) =>
				<IAzureQuickPickItem<UnifiedRegistryItem<CommonRegistry>>>{
					label: acr.wrappedItem.label,
					data: acr,
				},
		);
		picks.push({
		...
		const response = await wizardContext.ui.showQuickPick(picks, {
			placeHolder: vscode.l10n.t(
				"Select an Azure Container Registry to push to",
			),
		});

		if (response.data === "create") {
			const createdAcrName = await createAzureRegistry(
				wizardContext,
				wizardContext.azureSubscriptionNode,
			);
			...
			wizardContext.connectedRegistry =
				response.data as UnifiedRegistryItem<CommonRegistry>;
		}
	}
