...
export async function removeImageGroup(
	context: IActionContext,
	node?: ImageGroupTreeItem,
	nodes?: ImageGroupTreeItem[],
): Promise<void> {
	nodes = await multiSelectNodes(
		{
			...
			noItemFoundErrorMessage: vscode.l10n.t(
				"No image groups are available to remove",
			),
		},
		ext.imagesTree,
	...
	if (nodes.length === 1) {
		...
		confirmRemove = vscode.l10n.t(
			"Are you sure you want to remove the selected image groups? If there are other tags or child images under the selected groups, only the tags will be removed.",
		);
	}
