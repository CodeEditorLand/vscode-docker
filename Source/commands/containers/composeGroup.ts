...
export async function composeGroupLogs(
	context: IActionContext,
	node: ContainerGroupTreeItem,
): Promise<void> {
	// Since we're not interested in the output, we can pretend this is a `VoidCommandResponse`
	return composeGroup<LogsCommandOptions>(
		context,
		(client, options) =>
			client.logs(options) as Promise<VoidCommandResponse>,
		node,
		{ follow: true, tail: 1000 },
	);
}

export async function composeGroupStart(
	context: IActionContext,
	node: ContainerGroupTreeItem,
): Promise<void> {
	return composeGroup(
		context,
		(client, options) => client.start(options),
		node,
	);
}

export async function composeGroupStop(
	context: IActionContext,
	node: ContainerGroupTreeItem,
): Promise<void> {
	return composeGroup(
		context,
		(client, options) => client.stop(options),
		node,
	);
}

export async function composeGroupRestart(
	context: IActionContext,
	node: ContainerGroupTreeItem,
): Promise<void> {
	return composeGroup(
		context,
		(client, options) => client.restart(options),
		node,
	);
}

export async function composeGroupDown(
	context: IActionContext,
	node: ContainerGroupTreeItem,
): Promise<void> {
	return composeGroup(
		context,
		(client, options) => client.down(options),
		node,
	);
}

type AdditionalOptions<TOptions extends CommonOrchestratorCommandOptions> =
	Omit<TOptions, keyof CommonOrchestratorCommandOptions>;

async function composeGroup<TOptions extends CommonOrchestratorCommandOptions>(
	context: IActionContext,
	composeCommandCallback: (
		client: IContainerOrchestratorClient,
		options: TOptions,
			...
			await ext.containersTree.showTreeItemPicker<ContainerGroupTreeItem>(
				/composeGroup$/i,
				{
					...
					noItemFoundErrorMessage: l10n.t(
						"No Docker Compose projects are running.",
					),
				},
			);
	}
...
function getComposeWorkingDirectory(
	node: ContainerGroupTreeItem,
): string | undefined {
...
function getComposeFiles(node: ContainerGroupTreeItem): string[] | undefined {
	...
	const container = (node.ChildTreeItems as ContainerTreeItem[]).find(
		(c) => c.labels?.["com.docker.compose.project.config_files"],
	);
