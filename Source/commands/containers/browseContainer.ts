...
function toBrowsablePort(port: PortBinding): BrowsablePort {
	...
	if (
		...
		host === "127.0.0.1" ||
		host === "::1" // IP is a common loopback binding (IPv4 or IPv6)
	) {
...
function dedupeBrowsablePorts(
	browsablePorts: BrowsablePort[],
): BrowsablePort[] {
	...
	for (const browsablePort of browsablePorts) {
		...
		if (
			results.some(
				(p) =>
					p.host === browsablePort.host &&
					p.containerPort === browsablePort.containerPort,
			)
		) {
			continue;
		}
...
export async function browseContainer(
	...
	const telemetryProperties = <BrowseTelemetryProperties>(
		...
		node = await ext.containersTree.showTreeItemPicker<ContainerTreeItem>(
			ContainerTreeItem.runningContainerRegExp,
			{
				...
				noItemFoundErrorMessage: vscode.l10n.t(
					"No running containers are available to open in a browser",
				),
			},
		);
	}
	...
	const browsablePorts: BrowsablePort[] = dedupeBrowsablePorts(
		dupedPossiblePorts.map(toBrowsablePort),
	);

	telemetryProperties.possiblePorts = browsablePorts
	...
	if (browsablePorts.length === 1) {
		...
		const preferredCommonPrivatePort: number | undefined =
			commonWebPorts.find((commonPort) =>
				browsablePorts.some(
					(browsablePort) =>
	...
	if (selectedPort === undefined) {
		const items: IAzureQuickPickItem<BrowsablePort>[] = browsablePorts.map(
			(port) => ({
		...
		const item = await context.ui.showQuickPick(items, {
			...
			placeHolder: vscode.l10n.t(
				"Select the container port to browse to.",
			),
		});
	...
	const protocol = commonSslPorts.some(
		(commonPort) => commonPort === selectedPort.containerPort,
	)
		? "https"
		: "http";
