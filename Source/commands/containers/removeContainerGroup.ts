...
export async function removeContainerGroup(
	context: IActionContext,
	node?: ContainerGroupTreeItem,
	nodes?: ContainerGroupTreeItem[],
): Promise<void> {
	nodes = await multiSelectNodes(
		{
			...
			noItemFoundErrorMessage: vscode.l10n.t(
				"No container groups are available to remove",
			),
		},
		ext.containersTree,
		node?.contextValue,
	...
	if (nodes.length === 1) {
		confirmRemove = vscode.l10n.t(
			'Are you sure you want to remove container group "{0}"?',
			nodes[0].label,
		);
	} else {
		confirmRemove = vscode.l10n.t(
			"Are you sure you want to remove selected container groups?",
		);
	}
