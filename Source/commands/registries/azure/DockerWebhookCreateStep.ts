...
export class DockerWebhookCreateStep extends AzureWizardExecuteStep<IAppServiceWizardContext> {
	...
	public async execute(
		...
		if (isAzureRepository(this.tagItem.parent.wrappedItem)) {
			const creatingNewWebhook: string = vscode.l10n.t(
			...
			vscode.window
				.showInformationMessage(
					vscode.l10n.t(
						'To set up a CI/CD webhook, open the page "{0}" and enter the URI to the created web app in your dockerhub account',
						dockerhubUri,
					),
					dockerhubPrompt,
				)
	...
	public shouldExecute(context: IAppServiceWizardContext): boolean {
		return (
			!!context.site &&
			(isAzureRepository(this.tagItem.parent.wrappedItem) ||
				isDockerHubRepository(this.tagItem.parent.wrappedItem))
		);
	}

	private async createWebhookForApp(
		context: IAppServiceWizardContext,
		site: Site,
		appUri: string,
