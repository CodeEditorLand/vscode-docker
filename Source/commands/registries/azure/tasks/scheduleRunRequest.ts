...
import type {
	DockerBuildRequest as AcrDockerBuildRequest,
	FileTaskRunRequest as AcrFileTaskRunRequest,
	OS as AcrOS,
	Run as AcrRun,
...
export async function scheduleRunRequest(
	...
	rootStrategy?: RootStrategy | undefined,
	...
	if (requestType === "DockerBuildRequest") {
		...
		fileItem = await quickPickYamlFileItem(
			...
			rootFolder,
			vscode.l10n.t(
				"To run a task from a .yaml file you must have yaml file in your VS Code workspace.",
			),
		);
	} else {
		throw new Error(
			vscode.l10n.t("Run Request Type Currently not supported."),
		);
	}

	const node: UnifiedRegistryItem<AzureRegistryItem> =
		await registryExperience<AzureRegistry>(context, {
	...
	try {
		...
		if (requestType === "DockerBuildRequest") {
			runRequest = {
				...
				dockerFilePath: path.relative(
					rootUri.fsPath,
					fileItem.absoluteFilePath,
				),
			};
		} else {
			runRequest = {
				type: "FileTaskRunRequest",
				taskFilePath: path.relative(
					rootUri.fsPath,
					fileItem.absoluteFilePath,
				),
				sourceLocation: uploadedSourceLocation,
		...
		const run =
			await azureRegistryClient.registries.beginScheduleRunAndWait(
				resourceGroup,
				registryItem.label,
				runRequest,
			);
...
async function quickPickImageName(
	context: IActionContext,
	rootFolder: vscode.WorkspaceFolder,
	dockerFileItem: Item | undefined,
): Promise<string> {
...
async function streamLogs(
	context: IActionContext,
	registryItem: AzureRegistryItem,
	run: AcrRun,
): Promise<void> {
	const azureRegistryClient = await createAzureContainerRegistryClient(
		registryItem.subscription,
