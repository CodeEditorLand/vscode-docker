...
export async function deleteAzureRegistry(
	context: IActionContext,
	node?: UnifiedRegistryItem<AzureRegistry>,
): Promise<void> {
	if (!node) {
	...
	const confirmDelete: string = l10n.t(
		'Are you sure you want to delete registry "{0}" and its associated images?',
		registryName,
	);
	...
	const message = l10n.t(
		'Successfully deleted registry "{0}".',
		registryName,
	);
	// don't wait
