...
import {
	AzureRegistry,
	isAzureTag,
} from "../../../tree/registries/Azure/AzureRegistryDataProvider";
import {
	getFullImageNameFromRegistryTagItem,
	getResourceGroupFromAzureRegistryItem,
} from "../../../tree/registries/registryTreeUtils";
...
export class DockerAssignAcrPullRoleStep extends AzureWizardExecuteStep<IAppServiceWizardContext> {
	...
	public constructor(
		private readonly tagTreeItem: UnifiedRegistryItem<CommonTag>,
	) {
		super();
	}
	...
	public async execute(
		...
		const registry = await crmClient.registries.get(
			getResourceGroupFromAzureRegistryItem(registryTreeItem.wrappedItem),
		...
		if (!registry?.id) {
			throw new Error(
				l10n.t(
					"Unable to get details from Container Registry {0}",
					registryTreeItem.wrappedItem.label,
				),
			);
		}
		...
		const acrPullRoleDefinition = (
			await azExtAzureUtils.uiUtils.listAllIterator(
				authClient.roleDefinitions.list(registry.id, {
					filter: `roleName eq 'AcrPull'`,
				}),
			)
		)[0];
		...
		if (!acrPullRoleDefinition?.id) {
			throw new Error(
				l10n.t(
					"Unable to get AcrPull role definition on subscription {0}",
		...
		const siteInfo = await appSvcClient.webApps.get(
		...
		if (!siteInfo?.identity?.principalId) {
			throw new Error(
				l10n.t(
					"Unable to get identity principal ID for web site {0}",
					context.site.name,
				),
			);
		}
		...
		const config = await appSvcClient.webApps.getConfiguration(
			context.site.resourceGroup,
			context.site.name,
		);
		...
		if (!config) {
			throw new Error(
				l10n.t(
					"Unable to get configuration for web site {0}",
					context.site.name,
				),
			);
		}
	...
	public shouldExecute(context: IAppServiceWizardContext): boolean {
		return (
			!!context.site &&
			isAzureTag(this.tagTreeItem.wrappedItem) &&
