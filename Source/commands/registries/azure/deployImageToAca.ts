...
export async function deployImageToAca(
	...
	const registry: UnifiedRegistryItem<CommonRegistry> = node.parent
		.parent as unknown as UnifiedRegistryItem<CommonRegistry>;
	...
	if (isAzureRegistry(registry.wrappedItem)) {
		...
		const logInInfo: LoginInformation =
			await registry.provider.getLoginInformation(registry.wrappedItem);

		if (!logInInfo?.username || !logInInfo?.secret) {
			throw new Error(
				vscode.l10n.t(
					'No credentials found for registry "{0}".',
					registry.wrappedItem.label,
				),
			);
		}
	...
	commandOptions.registryName = nonNullProp(
		parseDockerLikeImageName(commandOptions.image),
		"registry",
	);
