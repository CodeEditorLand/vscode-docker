...
export type NetCoreScaffoldingOptions =
	| NetCoreDebugScaffoldingOptions
	| NetCoreTaskScaffoldingOptions;
...
export class DockerDebugScaffoldingProvider
	implements IDockerDebugScaffoldingProvider
{
	public async initializeNetCoreForDebugging(
		context: DockerDebugScaffoldContext,
		options?: NetCoreScaffoldingOptions,
	): Promise<void> {
	...
	public async initializeNodeForDebugging(
		context: DockerDebugScaffoldContext,
		options?: NodeScaffoldingOptions,
	): Promise<void> {
		await this.initializeForDebugging(
	...
	public async initializePythonForDebugging(
		context: DockerDebugScaffoldContext,
		options?: PythonScaffoldingOptions,
	): Promise<void> {
		await this.initializeForDebugging(
	...
	private async initializeForDebugging(
		...
		provideDockerRunTasks: () => Promise<DockerRunTaskDefinition[]>,
	): Promise<void> {
		...
		for (const buildTask of buildTasks) {
			...
			overwrite =
				await DockerDebugScaffoldingProvider.addObjectWithOverwritePrompt(
					(_overwrite?: boolean) =>
						addTask(buildTask, context.folder, _overwrite),
					overwrite,
				);
		}
		...
		for (const runTask of runTasks) {
			...
			overwrite =
				await DockerDebugScaffoldingProvider.addObjectWithOverwritePrompt(
					(_overwrite?: boolean) =>
						addTask(runTask, context.folder, _overwrite),
					overwrite,
				);
		}
	...
	private static async addObjectWithOverwritePrompt(
		addMethod: (_overwrite?: boolean) => Promise<boolean>,
		overwrite?: boolean,
		...
		if (!added && overwrite === undefined) {
			...
			overwrite =
				overwriteMessageItem ===
				(await window.showWarningMessage(
					l10n.t(
						"Docker launch configurations and/or tasks already exist. Do you want to overwrite them?",
					),
					...[overwriteMessageItem, DialogResponses.no],
				));
...
export const dockerDebugScaffoldingProvider: IDockerDebugScaffoldingProvider =
	new DockerDebugScaffoldingProvider();
