...
export interface DockerDebugConfiguration
	extends NetCoreDockerDebugConfiguration,
		NodeDockerDebugConfiguration {
	platform?: DockerPlatform;
}

export interface DockerAttachConfiguration
	extends NetCoreDockerDebugConfiguration,
		NodeDockerDebugConfiguration {
	processName?: string;
...
export class DockerDebugConfigurationProvider
	implements DebugConfigurationProvider
{
		...
		return callWithTelemetryAndErrorHandling(
			...
			async (actionContext: IActionContext) => {
				...
				if (Object.keys(debugConfiguration).length === 0) {
					const newlyCreatedDebugConfig =
						await this.handleEmptyDebugConfig(
							folder,
							actionContext,
						);
					// if there is no debugConfiguration, we should return undefined to exit the debug session
					if (
						newlyCreatedDebugConfig.length === 0 ||
				...
				if (!debugConfiguration.request) {
					throw new Error(
						l10n.t(
							'The property "request" must be specified in the debug config.',
						),
					);
				}
	...
	private async resolveDebugConfigurationInternal(
		context: DockerDebugContext,
		originalConfiguration: DockerDebugConfiguration,
	): Promise<DockerDebugConfiguration | undefined> {
		...
		await helper.afterResolveDebugConfiguration?.(
			context,
			originalConfiguration,
		);
	...
	private async validateResolvedConfiguration(
		resolvedConfiguration: ResolvedDebugConfiguration,
	): Promise<void> {
	...
	private getHelper(platform: DockerPlatform): DebugHelper {
		...
		if (!helper) {
			throw new Error(
				l10n.t(
					"The platform '{0}' is not currently supported for Docker debugging.",
					platform,
				),
			);
		}
	...
	private async removeDebugContainerIfNeeded(
		context: IActionContext,
		configuration: ResolvedDebugConfiguration,
	): Promise<void> {
		if (
			(configuration?.dockerOptions?.removeContainerAfterDebug ?? true) && // removeContainerAfterDebug must be undefined or true
			configuration?.dockerOptions?.containerName && // containerName must be specified
			!configuration?.subProcessId
		) {
			// Must not have subProcessId, i.e. not a subprocess debug session (which is how Python does hot reload sessions)
			try {
				await ext.runWithDefaults((client) =>
					client.removeContainers({
						containers: [configuration.dockerOptions.containerName],
						force: true,
					}),
				);
			} catch {
	...
	private async outputPortsAtDebuggingIfNeeded(
		context: IActionContext,
		configuration: ResolvedDebugConfiguration,
	): Promise<void> {
		if (configuration?.dockerOptions?.containerName) {
			try {
				const inspectInfo = (
					await ext.runWithDefaults((client) =>
						client.inspectContainers({
							containers: [
								configuration.dockerOptions.containerName,
							],
						}),
					)
				)?.[0];
				...
				for (const binding of inspectInfo?.ports ?? []) {
					portMappings.push(
						`localhost:${binding.hostPort} => ${binding.containerPort}`,
					);
				}
	 ...
	 */
	private async handleEmptyDebugConfig(
		...
		const csProjUris = await resolveFilesOfPattern(folder, [
			CSPROJ_GLOB_PATTERN,
			FSPROJ_GLOB_PATTERN,
		]);
