...
export async function trustCertificateIfNecessary(
	...
	if (isWindows()) {
		if (!(await isCertificateTrusted())) {
			...
			void context.ui
				.showWarningMessage(
					...
					{
						modal: false,
						learnMoreLink: "https://aka.ms/vscode-docker-dev-certs",
					},
					trust,
				)
		...
		if (!(await isCertificateTrusted())) {
			const message = l10n.t(
				"The ASP.NET Core HTTPS development certificate is not trusted. To trust the certificate, run `dotnet dev-certs https --trust`.",
			);
...
export async function exportCertificateIfNecessary(
	projectFile: string,
...
export function getHostSecretsFolders(): {
	...
	if (isWindows()) {
		...
		if (appDataEnvironmentVariable === undefined) {
			throw new Error(
				l10n.t(
					"The environment variable 'AppData' is not defined. This variable is used to locate the HTTPS certificate and user secrets folders.",
				),
			);
		}
	}
...
export function getContainerSecretsFolders(
	platform: PlatformOS,
	userName: string | undefined,
): { containerCertificateFolder: string; containerUserSecretsFolder: string } {
...
async function exportCertificateAndSetPassword(
	projectFile: string,
	certificateExportPath: string,
): Promise<void> {
