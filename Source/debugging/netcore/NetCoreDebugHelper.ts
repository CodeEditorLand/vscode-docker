...
import {
	NetCoreTaskHelper,
	NetCoreTaskOptions,
} from "../../tasks/netcore/NetCoreTaskHelper";
...
import {
	DebugHelper,
	DockerDebugContext,
	DockerDebugScaffoldContext,
	inferContainerName,
	ResolvedDebugConfiguration,
	resolveDockerServerReadyAction,
} from "../DebugHelper";
import {
	DockerAttachConfiguration,
	DockerDebugConfiguration,
} from "../DockerDebugConfigurationProvider";
import {
	exportCertificateIfNecessary,
	getHostSecretsFolders,
	trustCertificateIfNecessary,
} from "./AspNetSslHelper";
import {
	installDebuggersIfNecessary,
	vsDbgInstallBasePath,
	VsDbgType,
} from "./VsDbgHelper";
...
export class NetCoreDebugHelper implements DebugHelper {
		...
		return [
			{
				...
				netCore: {
					appProject: unresolveWorkspaceFolder(
						options.appProject,
						context.folder,
					),
				},
	...
	public async resolveDebugConfiguration(
		...
		switch (debugConfiguration.request) {
			...
			default:
				throw Error(
					l10n.t(
						"Unknown request {0} specified in the debug config.",
	...
	private async resolveLaunchDebugConfiguration(
		...
		debugConfiguration.netCore.appProject =
			...
			); // This method internally checks the user-defined input first
		...
		const additionalProbingPathsArgs =
			NetCoreDebugHelper.getAdditionalProbingPathsArgs(platformOS);

		const containerAppOutput = this.inferAppContainerOutput(
			appOutput,
			platformOS,
		);
		...
		const dockerServerReadyAction = resolveDockerServerReadyAction(
			...
			configureSsl ||
				(await NetCoreTaskHelper.isWebApp(
					debugConfiguration.netCore.appProject,
				)), // For .NET Console we won't create a DockerServerReadyAction unless at least part of one is user-provided
		);
		...
		return {
			...
			args:
				...
				[additionalProbingPathsArgs, containerAppOutput].join(" "),
			cwd:
				debugConfiguration.cwd || platformOS === "Windows"
					? "C:\\app"
					: "/app",
			dockerOptions: {
				containerName: containerName,
				...
				removeContainerAfterDebug:
					debugConfiguration.removeContainerAfterDebug,
			},
			pipeTransport: {
			...
			sourceFileMap: debugConfiguration.sourceFileMap || {
				...
				"/app/Views": path.join(
					path.dirname(debugConfiguration.netCore.appProject),
					"Views",
				),
			},
		};
	...
	private async resolveAttachDebugConfiguration(
		context: DockerDebugContext,
		debugConfiguration: DockerAttachConfiguration,
	): Promise<ResolvedDebugConfiguration | undefined> {
		// Get Container Name if missing
		const containerName: string =
			debugConfiguration.containerName ??
			(await this.getContainerNameToAttach(context.actionContext));
		...
		return {
			...
			processName: debugConfiguration.processId
				? undefined
				: debugConfiguration.processName || "dotnet",
			pipeTransport: {
				pipeProgram: await ext.runtimeManager.getCommand(),
	...
	protected async inferAppOutput(
		debugConfiguration: DockerDebugConfiguration,
	): Promise<string> {
	...
	protected inferAppContainerOutput(
		appOutput: string,
	...
	protected async loadExternalInfo(
		context: DockerDebugContext,
		debugConfiguration: DockerDebugConfiguration,
	): Promise<{
		...
		return {
			...
			containerName: inferContainerName(
				debugConfiguration,
				context,
				context.folder.name,
			),
	...
	protected async getProjectProperties(
		debugConfiguration: DockerDebugConfiguration,
	): Promise<NetCoreProjectProperties> {
		const projectInfo = await getNetCoreProjectInfo(
		...
		if (projectInfo.length < 3) {
			throw new Error(
				l10n.t("Unable to determine assembly output path."),
			);
		}
	...
	private async acquireDebuggers(platformOS: PlatformOS): Promise<void> {
			...
			async () => {
				if (platformOS === "Windows") {
					await installDebuggersIfNecessary([
						{ runtime: "win7-x64", version: "latest" },
					]);
				} else {
		...
		const debuggerScriptPath = path.join(
	...
	private static getAdditionalProbingPathsArgs(
		platformOS: PlatformOS,
	): string {
		const additionalProbingPaths =
			platformOS === "Windows"
	...
	private async copyDebuggerToContainer(
		context: IActionContext,
		containerName: string,
		containerDebuggerDirectory: string,
		...
		await window.withProgress(
			{
				location: ProgressLocation.Notification,
				title: l10n.t(
					"Copying the .NET debugger to the container ({0} --> {1})...",
					vsDbgInstallBasePath,
					containerDebuggerDirectory,
				),
			},
			async () => {
				await ext.runWithDefaults((client) =>
	...
	private async isDebuggerInstalled(
		containerName: string,
		debuggerPath: string,
		containerOS: ContainerOS,
	): Promise<boolean> {
		...
		if (containerOS === "windows") {
			...
			containerCommandArgs = composeArgs(
				withArg("/C"),
				withQuotedArg(
					`IF EXIST "${debuggerPath}" (exit 0) else (exit 1)`,
				),
			)();
		} else {
			...
			containerCommandArgs = composeArgs(
				withArg("-c"),
				withQuotedArg(
					`if [ -f ${debuggerPath} ]; then exit 0; else exit 1; fi;`,
				),
			)();
		}
	...
	private async getContainerNameToAttach(
		context: IActionContext,
	): Promise<string> {
