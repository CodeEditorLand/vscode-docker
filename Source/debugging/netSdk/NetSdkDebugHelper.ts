...
import { commands, l10n, tasks, WorkspaceFolder } from "vscode";

import { ext } from "../../extensionVariables";
import {
	NetChooseBuildTypeContext,
	netContainerBuild,
} from "../../scaffolding/wizard/net/NetContainerBuild";
import {
	AllNetContainerBuildOptions,
	NetContainerBuildOptionsKey,
} from "../../scaffolding/wizard/net/NetSdkChooseBuildStep";
import { NetCoreTaskHelper } from "../../tasks/netcore/NetCoreTaskHelper";
import {
	NetSdkRunTaskDefinition,
	netSdkRunTaskProvider,
} from "../../tasks/netSdk/NetSdkRunTaskProvider";
import {
	normalizeArchitectureToRidArchitecture,
	normalizeOsToRidOs,
} from "../../tasks/netSdk/netSdkTaskUtils";
...
import {
	NetCoreDebugHelper,
	NetCoreDebugScaffoldingOptions,
	NetCoreProjectProperties,
} from "../netcore/NetCoreDebugHelper";
...
export class NetSdkDebugHelper extends NetCoreDebugHelper {
	public override async provideDebugConfigurations(
		...
		if (
			netCoreBuildContext?.containerBuildOption ===
	...
	public async afterResolveDebugConfiguration(
		context: DockerDebugContext,
		debugConfiguration: DockerDebugConfiguration,
	): Promise<void> {
	...
	protected override async loadExternalInfo(
		context: DockerDebugContext,
		debugConfiguration: DockerDebugConfiguration,
	): Promise<{
		...
		return {
			...
			platformOS:
				(await getDockerOSType()) === "windows" ? "Windows" : "Linux",
		};
	}

	protected override inferAppContainerOutput(
		appOutput: string,
		platformOS: PlatformOS,
	): string {
		return appOutput;
	...
	protected override async getProjectProperties(
		debugConfiguration: DockerDebugConfiguration,
		...
		if (projectInfo.length < 6 || !projectInfo[5]) {
			throw new Error(
				l10n.t(
					"Your current project configuration or .NET SDK version doesn't support SDK Container build. Please choose a compatible project or update .NET SDK.",
				),
			);
		}
	...
	private async normalizeAppOutput(
		unnormalizedContainerWorkingDirectory: string,
		isSdkContainerSupportEnabled: boolean,
	): Promise<string> {
