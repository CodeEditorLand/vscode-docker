...
class ServerReadyDetector implements DockerServerReadyDetector {
	...
	private async openExternalWithString(
		...
		if (!args || !args.containerName) {
			throw new Error(
				vscode.l10n.t(
					"No container name was resolved or provided to DockerServerReadyAction.",
				),
			);
		}

		await callWithTelemetryAndErrorHandling(
			"dockerServerReadyAction.serverReadyDetector.openExternalWithString",
	...
	private async getHostPortForContainerPort(
		...
		if (!hostPort) {
			throw new Error(
				vscode.l10n.t(
					"Could not determine host port mapped to container port {0} in container '{1}'.",
					containerPort,
					containerName,
				),
			);
		...
		switch (args.action || "openExternally") {
			...
			case "debugWithChrome":
				if (
					...
					void vscode.debug.startDebugging(session.workspaceFolder, {
						type: "chrome",
						name: "Chrome Debug",
						request: "launch",
						url: uri,
...
class DockerLogsTracker extends vscode.Disposable {
	...
	public constructor(
		...
		private readonly detector: DockerServerReadyDetector,
	) {
		super(() => {
			this.cts.cancel();
...
class DockerDebugAdapterTracker
	extends vscode.Disposable
	implements vscode.DebugAdapterTracker
{
...
class MultiOutputDockerServerReadyManager
	extends vscode.Disposable
	implements DockerServerReadyDetector
{
	...
	public constructor(session: vscode.DebugSession) {
		...
		if (
			...
			this.logsTracker = new DockerLogsTracker(
				configuration.dockerOptions.dockerServerReadyAction.containerName,
				this,
			);
		}
...
class DockerDebugAdapterTrackerFactory
	implements vscode.DebugAdapterTrackerFactory
{
	private static trackers: Map<string, MultiOutputDockerServerReadyManager> =
		new Map<string, MultiOutputDockerServerReadyManager>();
...
class DockerServerReadyDebugConfigurationProvider
	implements vscode.DebugConfigurationProvider
{
	...
	public resolveDebugConfiguration?(
		...
		debugConfiguration: ResolvedDebugConfiguration,
		token?: vscode.CancellationToken,
	): vscode.ProviderResult<vscode.DebugConfiguration> {
		if (
			debugConfiguration?.type &&
			debugConfiguration?.dockerOptions?.dockerServerReadyAction
		) {
...
export function registerServerReadyAction(
	...
	context.subscriptions.push(
		vscode.debug.registerDebugConfigurationProvider(
			"*",
			new DockerServerReadyDebugConfigurationProvider(context),
		),
	);
}
