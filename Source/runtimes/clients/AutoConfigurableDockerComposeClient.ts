...
export class AutoConfigurableDockerComposeClient
	extends DockerComposeClient
	implements AutoConfigurableClient
{
	private readonly composeConfigLazy = new AsyncLazy<ComposeConfig>(() =>
		this.detectComposeConfig(),
	);
...
export function isAutoConfigurableDockerComposeClient(
	maybeClient: IContainerOrchestratorClient,
): maybeClient is AutoConfigurableDockerComposeClient {
	return (
		isDockerComposeClient(maybeClient) &&
		typeof (maybeClient as AutoConfigurableDockerComposeClient)
			.slowConfigure === "function"
	);
}
