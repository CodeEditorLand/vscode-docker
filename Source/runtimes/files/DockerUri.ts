...
export class DockerUri {
	...
	public get uri(): vscode.Uri {
		const uri = vscode.Uri.parse("docker:///").with({
		...
		return this.options
			? uri.with({
					query: queryToURLSearchParams(this.options).toString(),
				})
			: uri;
	}
	...
	private constructor(
		...
		public readonly options?: DockerUriQuery,
	) {}
}
...
function queryFromURLSearchParams(
	...
	return {
		...
		fileType:
			(Number.parseInt(queryParams.get("fileType")) as vscode.FileType) ||
