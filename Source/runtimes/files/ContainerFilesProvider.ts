...
export class ContainerFilesProvider
	...
	implements vscode.FileSystemProvider
{
	...
	public async readFile(uri: vscode.Uri): Promise<Uint8Array> {
		...
		const targetStream =
			containerOS === "windows"
				? accumulator
				: tarUnpackStream(accumulator);

		const generator = ext.streamWithDefaults((client) =>
			client.readFile({
				container: dockerUri.containerId,
				path:
					containerOS === "windows"
	...
	public writeFile(
		...
		return callWithTelemetryAndErrorHandling(
			...
			async (actionContext) => {
				...
				if (containerOS === "windows") {
					...
					if (inspectInfo?.isolation === "hyperv") {
						throw new CommandNotSupportedError(
							"Writing files is not supported on Windows Hyper-V containers.",
						);
					}
				}
				...
				const fileStats = await ext.runWithDefaults((client) =>
					client.statPath({
