...
import {
	DockerHubRegistryDataProvider,
	GenericRegistryV2DataProvider,
	GitHubRegistryDataProvider,
...
export function registerTrees(): void {
	...
	ext.containersTreeView = vscode.window.createTreeView("dockerContainers", {
		treeDataProvider: ext.containersTree,
		canSelectMany: true,
	});
	...
	registerCommand(
		...
		async (context: IActionContext, node: AzExtTreeItem) =>
			ext.containersTree.loadMore(node, context),
	);
	...
	ext.networksTreeView = vscode.window.createTreeView("dockerNetworks", {
		treeDataProvider: ext.networksTree,
		canSelectMany: true,
	});
	...
	registerCommand(
		networksLoadMore,
		async (context: IActionContext, node: AzExtTreeItem) =>
			ext.networksTree.loadMore(node, context),
	);
	...
	ext.imagesTreeView = vscode.window.createTreeView("dockerImages", {
		treeDataProvider: ext.imagesTree,
		canSelectMany: true,
	});
	...
	registerCommand(
		imagesLoadMore,
		async (context: IActionContext, node: AzExtTreeItem) =>
			ext.imagesTree.loadMore(node, context),
	);

	const urtdp = new UnifiedRegistryTreeDataProvider(ext.context.globalState);
	ext.registriesRoot = urtdp;
	ext.registriesTreeView = vscode.window.createTreeView("dockerRegistries", {
		treeDataProvider: urtdp,
	...
	ext.volumesTreeView = vscode.window.createTreeView("dockerVolumes", {
	...
	registerCommand(
		volumesLoadMore,
		async (context: IActionContext, node: AzExtTreeItem) =>
			ext.volumesTree.loadMore(node, context),
	);
	...
	ext.contextsTreeView = vscode.window.createTreeView(
		"vscode-docker.views.dockerContexts",
		{ treeDataProvider: ext.contextsTree, canSelectMany: false },
	);
	ext.context.subscriptions.push(ext.contextsTreeView);
	registerCommand(
		contextsLoadMore,
		async (context: IActionContext, node: AzExtTreeItem) =>
			ext.contextsTree.loadMore(node, context),
	);
	...
	registerCommand(
		...
		async (context: IActionContext, node: OpenUrlTreeItem) =>
			node.openUrl(),
	);
...
function registerRegistryDataProviders(
	urtdp: UnifiedRegistryTreeDataProvider,
): void {
	const githubRegistryDataProvider = new GitHubRegistryDataProvider(
		ext.context,
	);
	...
	const dockerHubRegistryDataProvider = new DockerHubRegistryDataProvider(
		ext.context,
	);
	ext.context.subscriptions.push(
	...
	const azureRegistryDataProvider = new AzureRegistryDataProvider(
		ext.context,
	);
	ext.context.subscriptions.push(
	...
	const genericRegistryV2DataProvider = new GenericRegistryV2DataProvider(
		ext.context,
	);
	ext.context.subscriptions.push(
