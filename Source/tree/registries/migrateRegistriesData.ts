 ...
 */
export async function migrateRegistriesData(
	...
	const oldRegistries = ctx.globalState.get<ICachedRegistryProvider[]>(
	...
	for (const oldRegistry of oldRegistries) {
		...
		if (
			...
			const trackedRegistryStrings = ctx.globalState.get<string[]>(
		...
		if (oldRegistry.username) {
			await ctx.globalState.update(
				`BasicAuthProvider.${credentialStorageKey}.username`,
				oldRegistry.username,
			);
		}
...
export async function getRegistryPassword(
	cached: ICachedRegistryProvider,
...
function getRegistryPasswordKey(cached: ICachedRegistryProvider): string {
	return getPseudononymousStringHash(
		cached.id + cached.api + (cached.url || "") + (cached.username || ""),
	);
}
