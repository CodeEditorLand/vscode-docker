...
import {
	...
	isRepository,
	isTag,
} from "@microsoft/vscode-docker-registries/lib/clients/Common/models";
...
export type AzureRegistry = V2Registry &
	AzureRegistryItem & {
...
export function isAzureSubscriptionRegistryItem(
	item: unknown,
): item is AzureSubscriptionRegistryItem {
	return (
		!!item &&
		typeof item === "object" &&
		(item as AzureSubscriptionRegistryItem).type === "azuresubscription"
	);
}
...
export function isAzureRepository(item: unknown): item is AzureRepository {
	return (
		isRepository(item) && item.additionalContextValues?.includes("azure")
	);
}
...
export class AzureRegistryDataProvider
	...
	implements vscode.Disposable
{
	...
	public override async getRegistries(
		subscriptionItem: CommonRegistryItem,
	): Promise<AzureRegistry[]> {
	...
	public override getTreeItem(
		element: CommonRegistryItem,
	...
	public async deleteRepository(item: AzureRepository): Promise<void> {
		...
		if (!reponse.succeeded) {
			throw new Error(
				`Failed to delete repository: ${reponse.statusText}`,
			);
		}
	}
	...
	public async tryGetAdminCredentials(
		azureRegistry: AzureRegistry,
	): Promise<RegistryListCredentialsResult | undefined> {
