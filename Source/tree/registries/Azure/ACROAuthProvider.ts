...
export class ACROAuthProvider implements AuthenticationProvider {
	...
	public async getSession(
		...
		const oauthToken = await this.getOAuthTokenFromRefreshToken(
			refreshToken,
			this.registryUri,
			scopes.join(" "),
			this.subscription,
	...
	private async getOAuthTokenFromRefreshToken(
		refreshToken: string,
	...
	private async getRefreshTokenFromAccessToken(
		accessToken: string,
		registryUri: vscode.Uri,
		subscription: AzureSubscription,
	): Promise<string> {
		...
		// eslint-disable-next-line @typescript-eslint/naming-convention
		const response = await httpRequest<{ refresh_token: string }>(
			requestUrl.toString(),
			{
				method: "POST",
				headers: {
	...
	private async getRefreshToken(
		...
		if (
			...
			refreshToken = await this.getRefreshTokenFromAccessToken(
				accessToken,
				this.registryUri,
				this.subscription,
			);
