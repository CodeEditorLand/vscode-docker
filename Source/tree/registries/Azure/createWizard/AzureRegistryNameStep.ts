...
export class AzureRegistryNameStep extends AzureNameStep<IAzureRegistryWizardContext> {
	protected async isRelatedNameAvailable(
		context: IAzureRegistryWizardContext,
		name: string,
	): Promise<boolean> {
	...
	public async prompt(context: IAzureRegistryWizardContext): Promise<void> {
		...
		context.newRegistryName = (
			await context.ui.showInputBox({
				...
				validateInput: (name: string) =>
					validateRegistryName(name, client),
			})
		).trim();
		...
		context.relatedNameTask = this.generateRelatedName(
			context,
			context.newRegistryName,
			azExtAzureUtils.resourceGroupNamingRules,
		);
	}
...
async function validateRegistryName(
	name: string,
	...
	if (name.length < min || name.length > max) {
		...
		const nameStatus = await client.registries.checkNameAvailability({
			name,
