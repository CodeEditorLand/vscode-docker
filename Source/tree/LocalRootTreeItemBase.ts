...
import {
	...
	IVSCodeRemoteInfo,
	RemoteKind,
} from "../utils/getVSCodeRemoteInfo";
...
import {
	ITreeSettingsWizardContext,
	ITreeSettingWizardInfo,
} from "./settings/ITreeSettingsWizardContext";
...
export type LocalChildType<T extends AnyContainerObject> = new (
...
> = new (
	parent: LocalRootTreeItemBase<TItem, TProperty>,
	group: string,
	items: TItem[],
...
> extends AzExtParentTreeItem {
		...
		try {
			...
			if (!this._currentDockerStatus) {
				this._currentDockerStatus =
					(await runtimeInstallStatusProvider.isRuntimeInstalled())
						? "Installed"
						: "NotInstalled";
			}
			...
			return await this.getDockerErrorTreeItems(
				context,
				error,
				this._currentDockerStatus === "Installed",
			);
		}
	...
	public compareChildrenImpl(ti1: AzExtTreeItem, ti2: AzExtTreeItem): number {
		if (this.failedToConnect) {
			...
			if (
				ti1 instanceof this.childGroupType &&
				ti2 instanceof this.childGroupType
			) {
				if (
					this.groupBySetting === "CreatedTime" &&
					ti2.maxCreatedTime !== ti1.maxCreatedTime
				) {
					return ti2.maxCreatedTime - ti1.maxCreatedTime;
				}
			} else if (
				ti1 instanceof this.childType &&
				ti2 instanceof this.childType
			) {
				if (
					this.sortBySetting === "CreatedTime" &&
					ti2.createdTime !== ti1.createdTime
				) {
					return ti2.createdTime - ti1.createdTime;
				} else if (
					this.sortBySetting === "Size" &&
					ti1.size !== undefined &&
					ti2.size !== undefined
				) {
	...
	private async groupItems(items: TItem[]): Promise<AzExtTreeItem[]> {
		...
		if (this.groupBySetting === "None") {
			...
			for (const item of items) {
				const groupName: string | undefined = this.getPropertyValue(
					item,
					this.groupBySetting,
				);
	...
	public getTreeSetting<T extends string>(
		setting: string,
		...
		if (
			...
			settingInfo.properties.find(
				(propInfo) => propInfo.property === value,
			)
		) {
			return value;
		} else {
	...
	public getTreeArraySetting<T extends string>(
		setting: string,
		...
		if (
			...
			value.every(
				(v1) =>
					!!settingInfo.properties.find((v2) => v1 === v2.property),
			)
		) {
			return value;
		} else {
	...
	public async configureExplorer(context: IActionContext): Promise<void> {
		...
		if (wizardContext.info) {
			...
			/* eslint-disable-next-line @typescript-eslint/no-floating-promises */
			this.config.update(
				wizardContext.info.setting,
				wizardContext.newValue,
	...
	private async getDockerErrorTreeItems(
		context: IActionContext,
		error: unknown,
		...
		if (isCommandNotSupportedError(error)) {
			return [
				new GenericTreeItem(this, {
					label: l10n.t(
						"This view is not supported in the current context.",
					),
					contextValue: "contextNotSupported",
				}),
			];
		} else if (parsedError.isUserCancelledError) {
			return [
				new GenericTreeItem(this, {
					label: l10n.t("Changing context..."),
					contextValue: "changingContexts",
				}),
			];
		}
		...
		const result: AzExtTreeItem[] = dockerInstalled
			? [
					new GenericTreeItem(this, {
						label: l10n.t(
							"Failed to connect. Is {0} running?",
							(await ext.runtimeManager.getClient()).displayName,
						),
					...
					new GenericTreeItem(this, {
						label: l10n.t("  Error: {0}", parsedError.message),
						contextValue: "connectionError",
					}),
					new OpenUrlTreeItem(
						this,
						l10n.t("Additional Troubleshooting..."),
						"https://aka.ms/AA37qt2",
					),
				]
			: [
					new GenericTreeItem(this, {
						label: l10n.t(
							"Failed to connect. Is {0} installed?",
							(await ext.runtimeManager.getClient()).displayName,
						),
						contextValue: "connectionError",
						iconPath: new ThemeIcon(
	...
	private showDockerInstallNotificationIfNeeded(): void {
		if (
			!dockerInstallNotificationShownToUser &&
			this._currentDockerStatus === "NotInstalled"
		) {
...
export function getTreeId(object: AnyContainerObject): string {
	const objectName =
		object.name ||
		(object as ListImagesItem).image?.originalName ||
		"<none>";
