...
export type DirectoryItemProvider = (
...
export class DirectoryTreeItem extends AzExtParentTreeItem {
	...
	public constructor(
		...
		private readonly containerOSProvider: (
	...
	public async loadMoreChildrenImpl(
		...
		return items.map((item) =>
			this.createTreeItemForDirectoryItem(item, actualUri),
		);
	}

	public compareChildrenImpl(
		item1: DirectoryTreeItem | FileTreeItem,
		item2: DirectoryTreeItem | FileTreeItem,
	): number {
		if (
			(item1 instanceof DirectoryTreeItem &&
				item2 instanceof DirectoryTreeItem) ||
	...
	private createTreeItemForDirectoryItem(
		...
		switch (fileType) {
			...
			case vscode.FileType.File:
				return new FileTreeItem(
					this,
					name,
					itemUri.with({ fileType: vscode.FileType.File }),
				);

			default:
				throw new Error(
					vscode.l10n.t("Unrecognized directory item type."),
				);
		}
	}
