...
export class ContainersTreeItem extends LocalRootTreeItemBase<
	DockerContainerInfo,
	ContainerProperty
> {
	...
	public groupBySettingInfo: ITreeSettingInfo<
		ContainerProperty | CommonGroupBy
	> = {
	...
	public compareChildrenImpl(
		ti1: ContainerTreeItem,
		ti2: ContainerTreeItem,
	): number {
		...
		if (
			this.groupBySetting === "Compose Project Name" &&
			ti1 instanceof this.childGroupType &&
			ti2 instanceof this.childGroupType &&
			(ti1.label === NonComposeGroupName ||
				ti2.label === NonComposeGroupName)
		) {
	...
	protected getTreeItemForEmptyList(): AzExtTreeItem[] {
		if (this.newContainerUser) {
			const dockerTutorialTreeItem = new OpenUrlTreeItem(
				this,
	...
	private async updateNewContainerUser(
