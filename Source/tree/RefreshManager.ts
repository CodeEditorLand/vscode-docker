...
const ContainerEventActions: EventAction[] = [
	"create",
	"destroy",
	"die",
	"kill",
...
export class RefreshManager extends vscode.Disposable {
	...
	private setupAutoRefreshes(): void {
		...
		this.autoRefreshDisposables.unshift(
			new vscode.Disposable(() => this.cts.cancel()),
		);
		...
		void callWithTelemetryAndErrorHandling(
			...
			async (context: IActionContext) => {
				...
				while (errorCount < eventListenerTries) {
					...
					try {
						const eventGenerator = ext.streamWithDefaults(
							(client) =>
						...
						if (
							...
							ext.outputChannel.error(
								vscode.l10n.t(
									"Failed to set up event listener: {0}",
									error.message,
								),
							);
		...
		void callWithTelemetryAndErrorHandling(
			...
			async (context: IActionContext) => {
				...
				try {
					...
					this.autoRefreshDisposables.push(
						contextWatcher.onDidCreate(async () => {
							await this.refresh("contexts", "event");
						}),
					);

					this.autoRefreshDisposables.push(
						contextWatcher.onDidDelete(async () => {
							await this.refresh("contexts", "event");
						}),
					);
				} catch {
