...
export class ImagesTreeItem extends LocalRootTreeItemBase<
	DatedDockerImage,
	ImageProperty
> {
	private readonly outdatedImageChecker: OutdatedImageChecker =
		new OutdatedImageChecker();
	...
	public groupBySettingInfo: ITreeSettingInfo<ImageProperty | CommonGroupBy> =
		{
			// No grouping by size
			properties: [
				...imageProperties.filter((p) => p.property !== "Size"),
				groupByNoneProperty,
			],
	...
	): Promise<DatedDockerImage[]> {
		const includeDangling = ext.context.globalState.get(
