...
export const imageProperties: ITreePropertyInfo<ImageProperty>[] = [
	...
	{
		property: "FullTag",
		exampleValue: "example.azurecr.io/hello-world:latest",
	},
	{ property: "ImageId", exampleValue: "d9d09edd6115" },
	{ property: "Registry", exampleValue: "example.azurecr.io" },
	{
		property: "RegistryAndPath",
		exampleValue: "example.azurecr.io/my-path/hello-world",
	},
...
export function getImageGroupIcon(
	property: ImageProperty | CommonGroupBy,
): ThemeIcon {
	switch (property) {
		case "Registry":
...
export function getImagePropertyValue(
	...
	if (item.image.registry) {
		result = result.replace(
			item.image.registry,
			truncateRegistry(item.image.registry),
		);
	}
