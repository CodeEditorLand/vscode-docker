...
import {
	...
	CSPROJ_GLOB_PATTERN,
	DOCKERFILE_GLOB_PATTERN,
	FILE_SEARCH_MAX_RESULT,
	FSPROJ_GLOB_PATTERN,
	YAML_GLOB_PATTERN,
} from "../constants";
...
export function createFileItem(
	...
	const relativeFilePath = path.join(
		".",
		uri.fsPath.substr(rootFolder.uri.fsPath.length),
	);
...
export async function resolveFilesOfPattern(
	rootFolder: vscode.WorkspaceFolder,
	filePatterns: string[],
	excludePattern?: string,
	...
	await Promise.all(
		filePatterns.map(async (pattern: string) => {
			uris.push(
				...(await getFileUris(rootFolder, pattern, excludePattern)),
			);
		}),
	);
	// de-dupe
	uris = uris.filter(
		(uri, index) =>
			uris.findIndex((uri2) => uri.toString() === uri2.toString()) ===
			index,
	);
...
async function quickPickFileItem(
	...
	if (items) {
		if (items.length === 1) {
			...
			return await context.ui.showQuickPick<Item>(items, {
				placeHolder: message,
			});
		}
	}
...
export async function quickPickDockerFileItem(
	context: IActionContext,
	...
	while (!selectedDockerFile) {
		const dockerFiles: Item[] | undefined = await resolveFilesOfPattern(
			rootFolder,
			globPatterns,
		);
...
export async function quickPickDockerComposeFileItem(
	context: IActionContext,
	rootFolder: vscode.WorkspaceFolder,
	message: string,
): Promise<Item | undefined> {
	...
	while (!selectedComposeFile) {
		const composeFiles: Item[] | undefined = await resolveFilesOfPattern(
			rootFolder,
			globPatterns,
		);
		if (composeFiles) {
			if (
				(composeFiles.length === 1 &&
					isDefaultDockerComposeFile(composeFiles[0].label)) ||
				(composeFiles.length === 2 &&
					composeFiles.some((i) =>
						isDefaultDockerComposeFile(i.label),
					) &&
					composeFiles.some((i) =>
						isDefaultDockerComposeOverrideFile(i.label),
					))
			) {
				...
				selectedComposeFile = await quickPickFileItem(
					context,
					composeFiles,
					message,
				);
...
export async function quickPickYamlFileItem(
	context: IActionContext,
	fileUri: vscode.Uri,
	rootFolder: vscode.WorkspaceFolder,
	noYamlFileMessage: string,
): Promise<Item> {
	if (fileUri) {
	...
	const fileItem: Item = await quickPickFileItem(
		context,
		items,
		vscode.l10n.t("Choose a .yaml file to run."),
	);
...
export async function quickPickProjectFileItem(
	context: IActionContext,
	fileUri: vscode.Uri,
	rootFolder: vscode.WorkspaceFolder,
	noProjectFileMessage: string,
): Promise<Item> {
	if (fileUri) {
		return createFileItem(rootFolder, fileUri);
	}
