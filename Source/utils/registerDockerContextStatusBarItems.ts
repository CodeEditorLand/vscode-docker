...
export function registerDockerContextStatusBarEvent(
	ctx: vscode.ExtensionContext,
): void {
	// Register an event to watch for changes to config, reconfigure if needed
	registerEvent(
		"docker.context.showInStatusBar.changed",
	...
	ctx.subscriptions.push(
		...
		ext.runtimeManager.contextManager.onContextChanged(
...
async function showStatusBarItemIfNeeded() {
	...
	if (
		!config.get(dockerContextStatusBarSetting, false) ||
		!(currentDockerContext =
			await ext.runtimeManager.contextManager.getCurrentContext())
	) {
		// Intentional assignment and boolean check
		return;
	}

	const dockerContextUseCommand = "vscode-docker.contexts.use";

	ext.dockerContextStatusBarItem = vscode.window.createStatusBarItem(
...
function scheduleUpdateStatusBar(): Promise<void> {
	if (!updatePromise) {
		updatePromise = showStatusBarItemIfNeeded().finally(
