...
export async function inferPackageName(
	nodePackage: NodePackage | undefined,
	packagePath: string,
): Promise<string> {
...
export async function inferCommand(
			...
			if (startScript) {
				...
				if (result) {
					...
					return [
						...
						inspectArgWithPort,
						startScript
							.slice(result.index + capturedString.length)
							.trim(),
					];
				}
			}
	...
	throw new Error(
		l10n.t(
			"Unable to infer the command to run the application within the container. Set the 'dockerRun.command' property and include the Node.js '{0}' argument.",
			inspectArgWithPort,
		),
	);
}
