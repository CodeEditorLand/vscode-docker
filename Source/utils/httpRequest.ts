...
export class HttpResponse<T> implements ResponseLike {
	...
	public constructor(
		private readonly innerResponse: Response,
		...
		this.headers = {
			...
			set: (key: string, value: string) => {
				this.innerResponse.headers.set(key, value);
			},
		};
...
): IOAuthContext | undefined {
	if (error.response?.status === HttpStatusCode.Unauthorized) {
		const wwwAuthHeader: string | undefined = error.response?.headers?.get(
			"www-authenticate",
		) as string;
