...
export async function updateBlazorManifest(
	context: DockerRunTaskContext,
	runDefinition: DockerRunTaskDefinition,
): Promise<void> {
	const contents = await getNetCoreProjectInfo(
		"GetBlazorManifestLocations",
	...
	if (contents.length < 2) {
		throw new Error(
			l10n.t(
				"Unable to determine Blazor manifest locations from output file.",
			),
		);
	}

	await transformBlazorManifest(
		context,
		contents[0].trim(),
		contents[1].trim(),
		runDefinition.dockerRun.volumes,
		runDefinition.dockerRun.os,
	);
}

async function transformBlazorManifest(
	context: DockerRunTaskContext,
	...
	if (
		!inputManifest || // Input manifest can't be empty/undefined
	...
	context.terminal.writeOutputLine(
		l10n.t(
			"Attempting to containerize Blazor static web assets manifest...",
		),
	);

	if (path.extname(inputManifest) === ".json") {
		await transformJsonBlazorManifest(
			inputManifest,
			outputManifest,
			volumes,
			os,
		...
		await transformXmlBlazorManifest(
			inputManifest,
			outputManifest,
			volumes,
			os,
...
async function transformJsonBlazorManifest(
	inputManifest: string,
	outputManifest: string,
	volumes: DockerContainerVolume[],
	os: PlatformOS,
	...
	if (!manifest?.ContentRoots) {
		throw new Error(
			l10n.t("Failed to parse Blazor static web assets manifest."),
		);
	}
	...
	manifest.ContentRoots = manifest.ContentRoots.map((cr) =>
		tryContainerizePath(cr, volumes, os),
	);
...
async function transformXmlBlazorManifest(
	inputManifest: string,
	outputManifest: string,
	volumes: DockerContainerVolume[],
	os: PlatformOS,
	...
	for (const contentRoot of manifest.StaticWebAssets.ContentRoot) {
		if (contentRoot && contentRoot.$) {
			contentRoot.$.Path = tryContainerizePath(
				contentRoot.$.Path,
				volumes,
				os,
			);
...
function tryContainerizePath(
	oldPath: string,
	volumes: DockerContainerVolume[],
	os: PlatformOS,
): string {
	const matchingVolume: DockerContainerVolume = volumes.find((v) =>
		oldPath.toLowerCase().startsWith(v.localPath.toLowerCase()),
	);

	return matchingVolume
		? pathNormalize(
				matchingVolume.containerPath +
					oldPath.substring(matchingVolume.localPath.length),
				os,
			)
		: oldPath;
}
