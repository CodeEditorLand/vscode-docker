...
import {
	getContainerSecretsFolders,
	getHostSecretsFolders,
} from "../../debugging/netcore/AspNetSslHelper";
...
import {
	resolveVariables,
	unresolveWorkspaceFolder,
} from "../../utils/resolveVariables";
import {
	DockerBuildOptions,
	DockerBuildTaskDefinitionBase,
} from "../DockerBuildTaskDefinitionBase";
import { DockerBuildTaskDefinition } from "../DockerBuildTaskProvider";
import {
	DockerContainerVolume,
	DockerRunOptions,
	DockerRunTaskDefinitionBase,
} from "../DockerRunTaskDefinitionBase";
import { DockerRunTaskDefinition } from "../DockerRunTaskProvider";
import {
	addVolumeWithoutConflicts,
	DockerBuildTaskContext,
	DockerRunTaskContext,
	DockerTaskContext,
	DockerTaskScaffoldContext,
...
export interface NetCoreBuildTaskDefinition
...
export class NetCoreTaskHelper implements TaskHelper {
	public async provideDockerBuildTasks(
		context: DockerTaskScaffoldContext,
		...
		return [
			{
				...
				dockerBuild: {
					...
					target: "base",
					dockerfile: unresolveWorkspaceFolder(
						context.dockerfile,
						context.folder,
					),
	...
	public async provideDockerRunTasks(
		context: DockerTaskScaffoldContext,
		options?: NetCoreTaskScaffoldingOptions,
		...
		const configureSsl: false | undefined =
			context.ports?.length === 1 && context.ports?.[0] === 80
				? false
				: undefined;
		...
		return [
			{
				...
				dockerRun: {
					os:
						options.platformOS === "Windows"
							? "Windows"
							: undefined, // Default is Linux so we'll leave it undefined for brevity
				},
				netCore: {
					appProject: unresolveWorkspaceFolder(
						options.appProject,
	...
	public async getDockerBuildOptions(
		context: DockerBuildTaskContext,
		buildDefinition: NetCoreBuildTaskDefinition,
	...
	public async getDockerRunOptions(
		...
		context.actionContext.telemetry.properties.netCoreSslSetting =
		...
		runOptions.volumes = await this.inferVolumes(
			context.folder,
			runOptions,
			helperOptions,
			ssl,
			userSecrets,
		); // Volumes specifically are unioned with the user input (their input does not override except where the container path is the same)
	...
	public async postRun(
		...
		try {
			if (
				runDefinition.netCore.enableDebugging &&
				(await NetCoreTaskHelper.isWebApp(
					runDefinition.netCore.appProject,
				))
			) {
			...
			context.terminal.writeWarningLine(
				l10n.t(
					"Failed to update Blazor static web assets manifest. Static web assets may not work.\nThe error was: {0}",
	...
	public static async inferAppProject(
		context: DockerTaskContext,
		helperOptions?: NetCoreTaskOptions | NetCoreDebugOptions,
	): Promise<string> {
		let result: string;
		...
		if (helperOptions && helperOptions.appProject) {
			...
			const item = await quickPickProjectFileItem(
	...
	public static async isWebApp(appProject: string): Promise<boolean> {
		...
		return /Sdk\s*=\s*"Microsoft\.NET\.Sdk\.Web"/gi.test(
			projectContents.toString(),
		);
	}

	private async inferUserSecrets(
		helperOptions: NetCoreTaskOptions,
	): Promise<boolean> {
	...
	private async inferVolumes(
		...
		if (helperOptions.enableDebugging) {
			...
			const nugetUserVolume: DockerContainerVolume = {
				...
				containerPath:
					runOptions.os === "Windows"
						? "C:\\Users\\ContainerUser\\.nuget\\packages"
						: "/home/appuser/.nuget/packages",
				permissions: "ro",
			};
		...
		if (userSecrets || ssl) {
			...
			const userSecretsVolume: DockerContainerVolume = {
				...
				containerPath:
					containerSecretsFolders.containerUserSecretsFolder,
				permissions: "ro",
			};
			...
			if (ssl) {
				const certVolume: DockerContainerVolume = {
					...
					containerPath:
						containerSecretsFolders.containerCertificateFolder,
					permissions: "ro",
				};
