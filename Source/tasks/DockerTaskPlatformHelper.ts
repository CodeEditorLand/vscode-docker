...
export function getTaskPlatform(
	definition: DockerBuildTaskDefinition | DockerRunTaskDefinition,
): DockerPlatform | undefined {
	if (definition.platform === "netCore" || definition.netCore !== undefined) {
		return "netCore";
	} else if (
		definition.platform === "node" ||
		definition.node !== undefined
	) {
		return "node";
	} else if (
		definition.platform === "python" ||
		definition.python !== undefined
	) {
		return "python";
	} else {
		return undefined;
