...
import {
	getValidImageName,
	getValidImageNameWithTag,
} from "../utils/getValidImageName";
...
import {
	...
	DockerBuildTaskProvider,
} from "./DockerBuildTaskProvider";
...
import {
	DockerRunTask,
	DockerRunTaskDefinition,
	DockerRunTaskProvider,
} from "./DockerRunTaskProvider";
...
export type DockerTaskProviderName =
	| "docker-build"
	| "docker-run"
	| "docker-compose"
...
export async function addTask(
	newTask: DockerBuildTaskDefinition | DockerRunTaskDefinition,
	folder: WorkspaceFolder,
	overwrite?: boolean,
): Promise<boolean> {
...
export async function getAssociatedDockerRunTask(
	...
	return (await recursiveFindTaskByType(
		allTasks,
		"docker-run",
		debugConfiguration,
	)) as DockerRunTaskDefinition;
}

export async function getAssociatedDockerBuildTask(
	runTask: DockerRunTask,
	folder: WorkspaceFolder,
	...
	const runTaskDefinition: DockerRunTaskDefinitionBase =
		await findTaskByLabel(allTasks, runTask.name);

	return (await recursiveFindTaskByType(
		allTasks,
		"docker-build",
		runTaskDefinition,
	)) as DockerBuildTaskDefinition;
}

export async function getOfficialBuildTaskForDockerfile(
	context: IActionContext,
	dockerfile: string,
	folder: WorkspaceFolder,
): Promise<Task | undefined> {
	const resolvedDockerfile = pathNormalize(
		resolveVariables(dockerfile, folder),
	);
	...
	buildTasks = buildTasks.filter((buildTask) => {
		const taskDockerfile = pathNormalize(
			resolveVariables(
				buildTask.definition?.dockerBuild?.dockerfile ?? "Dockerfile",
				folder,
			),
		);
		...
		if (taskDockerfile && taskContext) {
			const taskDockerfileAbsPath = path.resolve(
				taskContext,
				taskDockerfile,
			);
			return (
	...
	if (buildTasks.length === 1) {
		...
		const releaseTask = buildTasks.find(
			(t) => t.name === "docker-build: release",
		);
...
export function inferImageName(
	runOptions: DockerRunTaskDefinition,
	context: DockerRunTaskContext,
	defaultNameHint: string,
	defaultTag?: "dev" | "latest",
): string {
	return (
		(runOptions && runOptions.dockerRun && runOptions.dockerRun.image) ||
		(context &&
			context.buildDefinition &&
			context.buildDefinition.dockerBuild &&
			context.buildDefinition.dockerBuild.tag) ||
		getDefaultImageName(defaultNameHint, defaultTag)
	);
}

export function getDefaultImageName(
	nameHint: string,
	tag?: "dev" | "latest",
): string {
	tag = tag || "latest";
...
export function getDefaultContainerName(
	nameHint: string,
	tag?: "dev" | "latest",
): string {
	tag = tag || "dev";
...
export async function recursiveFindTaskByType(
	allTasks: TaskDefinitionBase[],
	type: string,
	node: DebugConfigurationBase | TaskDefinitionBase,
): Promise<TaskDefinitionBase | undefined> {
	if (!node) {
		return undefined;
	}
...
export function addVolumeWithoutConflicts(
	volumes: DockerContainerVolume[],
	volume: DockerContainerVolume,
): boolean {
...
async function findTaskByLabel(
	allTasks: TaskDefinitionBase[],
	label: string,
): Promise<TaskDefinitionBase | undefined> {
...
export function normalizePlatform(
	platform: string | ContainerPlatform,
	...
	if (platform && typeof platform === "string") {
		...
		if (!os || !architecture) {
			throw new Error(
				'Platform string is malformed. It should be in the format "{os}/{architecture}".',
			);
		}
