...
import {
	...
	getAssociatedDockerBuildTask,
	TaskHelper,
	throwIfCancellationRequested,
} from "./TaskHelper";

export interface DockerRunTaskDefinition
...
export class DockerRunTaskProvider extends DockerTaskProvider {
	public constructor(helpers: { [key in DockerPlatform]: TaskHelper }) {
		super("docker-run", helpers);
	}
	...
	protected async executeTaskInternal(
		...
		context.actionContext.telemetry.properties.containerOS =
			definition.dockerRun.os || "Linux";

		context.buildDefinition = await getAssociatedDockerBuildTask(
			task,
			context.folder,
		);
		...
		if (helper) {
			definition.dockerRun = await helper.getDockerRunOptions(
				context,
				definition,
			);
			throwIfCancellationRequested(context);
		...
		const command = await client.runContainer({
			detached: true,
			publishAllPorts:
				options.portsPublishAll ||
				(options.portsPublishAll === undefined &&
					(options.ports === undefined || options.ports.length < 1)),
	...
	private async validateResolvedDefinition(
		...
		if (!dockerRun.image) {
			throw new Error(
				l10n.t("No Docker image name was provided or resolved."),
			);
		}
	}
	...
	private getMounts(
		...
		return volumes?.map((v) => {
			return {
				...
				readOnly:
					v.permissions === "ro" ||
					(v.permissions as unknown) === "ro,z", // Maintain compatibility with old `ro,z` option as much as possible
				type: "bind",
			};
		});
	}
