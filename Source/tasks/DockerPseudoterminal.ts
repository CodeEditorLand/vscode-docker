...
export class DockerPseudoterminal implements Pseudoterminal {
	...
	public constructor(
		private readonly taskProvider: DockerTaskProvider,
		private readonly task: DockerBuildTask | DockerRunTask,
		private readonly resolvedDefinition:
			| DockerBuildTaskDefinition
			| DockerRunTaskDefinition,
	) {}
	...
	public open(initialDimensions: TerminalDimensions | undefined): void {
		const folder =
			this.task.scope === TaskScope.Workspace
		...
		// Can't wait here
		void this.taskProvider
			.executeTask(executeContext, this.task)
	...
	public getCommandRunner(
		options: Omit<
			ExecuteCommandResponseInTerminalOptions,
			"commandResponse"
		>,
	): <T>(
		commandResponse: VoidCommandResponse | PromiseCommandResponse<T>,
	) => Promise<T> {
		return async <T>(
			commandResponse: VoidCommandResponse | PromiseCommandResponse<T>,
		) => {
	...
	private async executeCommandResponseInTerminal(
		options: ExecuteCommandResponseInTerminalOptions,
	...
	public async execAsyncInTerminal(
		command: string,
		options?: ExecAsyncInTerminalOptions,
	): Promise<ExecAsyncOutput> {
		...
		return await execAsync(
			...
			{
				cwd:
					this.resolvedDefinition.options?.cwd ||
					options.cwd ||
					options.folder.uri.fsPath,
				env: withDockerEnvSettings({
					...process.env,
					...this.resolvedDefinition.options?.env,
				}),
