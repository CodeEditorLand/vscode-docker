...
import {
	DockerTaskExecutionContext,
	DockerTaskProviderName,
	TaskHelper,
} from "./TaskHelper";

export abstract class DockerTaskProvider implements TaskProvider {
	protected constructor(
		private readonly telemetryName: DockerTaskProviderName,
		protected readonly helpers: { [key in DockerPlatform]: TaskHelper },
	) {}
	...
	public resolveTask(
		...
		return new Task(
			...
			task.name,
			task.source,
			new CustomExecution(async (resolvedDefinition: TaskDefinition) =>
				Promise.resolve(
					new DockerPseudoterminal(this, task, resolvedDefinition),
				),
			),
	...
	public async executeTask(
		context: DockerTaskExecutionContext,
		task: DockerBuildTask | DockerRunTask,
	): Promise<number> {
		try {
			await callWithTelemetryAndErrorHandling(
				`${this.telemetryName}-execute`,
				async (actionContext: IActionContext) => {
					...
					ext.activityMeasurementService.recordActivity(
						"overallnoedit",
					);
					...
					if (!context.folder) {
						throw new Error(
							l10n.t(
								"Unable to determine task scope to execute {0} task '{1}'. Please open a workspace folder.",
								this.telemetryName,
								task.name,
							),
						);
	...
	protected abstract executeTaskInternal(
		context: DockerTaskExecutionContext,
		task: Task,
	): Promise<void>;
