...
export class PythonDebugHelper implements DebugHelper {
	public async provideDebugConfigurations(
		...
		const projectType =
			options.projectType.charAt(0).toUpperCase() +
			options.projectType.slice(1);
	...
	public async resolveDebugConfiguration(
		...
		const args = [
			...(debugConfiguration.python.args ||
				pythonRunTaskOptions.args ||
				[]),
		...
		return {
			...
			django:
				debugConfiguration.python.django || projectType === "django",
			fastapi:
				debugConfiguration.python.fastapi || projectType === "fastapi",
			...
			debugLauncherPath:
				debugConfiguration.debugLauncherPath || launcherPath,
			debugAdapterHost:
				debugConfiguration.debugAdapterHost ||
				(await this.tryGetDebugAdapterHost(context)) ||
				"172.17.0.1", // 172.17.0.1 is the default gateway for the bridge network
			...
			redirectOutput:
				(debugConfiguration.redirectOutput as boolean | undefined) ??
				true,
			args: args,
			cwd: ".",
	...
	private async tryGetDebugAdapterHost(
		context: DockerDebugContext,
