...
import {
	...
	DockerDebugConfigurationProvider,
...
export async function addDebugConfiguration(
	newConfig: DockerDebugConfiguration,
	folder: WorkspaceFolder,
	overwrite?: boolean,
): Promise<boolean> {
...
export function inferContainerName(
	...
	return (
		(debugConfiguration && debugConfiguration.containerName) ||
		(context &&
			context.runDefinition &&
			context.runDefinition.dockerRun &&
			context.runDefinition.dockerRun.containerName) ||
...
export function resolveDockerServerReadyAction(
	debugConfiguration: DockerDebugConfiguration,
	defaultDockerSRA: DockerServerReadyAction,
	createIfUserUndefined: boolean,
): DockerServerReadyAction | undefined {
	const numBrowserOptions = [
		debugConfiguration.launchBrowser,
	...
	if (numBrowserOptions > 1) {
		...
		throw new Error(
			l10n.t(
				"Only at most one of the 'launchBrowser', 'serverReadyAction', and 'dockerServerReadyAction' properties may be set at a time.",
			),
		);
	} else if (
		numBrowserOptions === 1 &&
	...
	return {
		containerName:
			providedDockerSRA.containerName || defaultDockerSRA.containerName,
