...
export async function installDebuggersIfNecessary(
	debuggers: VsDbgType[],
): Promise<void> {
	...
	await Promise.all(
		debuggers.map((d) =>
			executeAcquisitionScriptIfNecessary(
				d.runtime,
				d.version,
				newScript,
			),
...
async function getLatestAcquisitionScriptIfNecessary(): Promise<boolean> {
	const lastAcquired = ext.context.globalState.get<number | undefined>(
		scriptAcquiredDateKey,
		undefined,
	);

	if (
		lastAcquired &&
		Date.now() - lastAcquired < dayInMs &&
		(await fse.pathExists(acquisition.scriptPath))
	) {
...
async function executeAcquisitionScriptIfNecessary(
	runtime: VsDbgRuntime,
	version: VsDbgVersion,
	newScript: boolean,
): Promise<void> {
	...
	const lastExecuted = ext.context.globalState.get<number | undefined>(
		scriptExecutedDateKey,
		undefined,
	);
	...
	if (
		!newScript &&
		lastExecuted &&
		Date.now() - lastExecuted < dayInMs &&
		(await fse.pathExists(getInstallDirectory(runtime, version)))
	) {
	...
	ext.outputChannel.info(
		l10n.t(
			"Installing VsDbg, Runtime = {0}, Version = {1}...",
			runtime,
			version,
		),
	);
