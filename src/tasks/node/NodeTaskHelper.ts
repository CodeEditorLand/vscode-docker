...
export class NodeTaskHelper implements TaskHelper {
	...
	public async provideDockerRunTasks(
		...
		return [
			...
			},
			{
				...
				node: {
					// If the package is at the root, we'll leave it out of the config for brevity, otherwise it must be specified explicitly
					...NodeTaskHelper.getNodeOptionsForScaffolding(
						options?.package,
						context.folder,
					),
	...
	public async getDockerBuildOptions(
		...
		if (buildOptions.context === undefined) {
			buildOptions.context =
				NodeTaskHelper.inferBuildContextPath(packagePath);
		}

		if (buildOptions.dockerfile === undefined) {
			buildOptions.dockerfile =
				NodeTaskHelper.inferDockerfilePath(packagePath);
		}
	...
	public async getDockerRunOptions(
		...
		const packagePath = NodeTaskHelper.inferPackagePath(
			helperOptions && helperOptions.package,
			context.folder,
		);

		const nodePackage = new Lazy<Promise<NodePackage>>(async () => {
			return await readPackage(packagePath);
		});

		const packageName = await inferPackageName(
			await nodePackage.value,
			packagePath,
		);
		...
		if (helperOptions && helperOptions.enableDebugging) {
			...
			if (runOptions.command === undefined) {
				runOptions.command = await inferCommand(
					await nodePackage.value,
					inspectMode,
					inspectPort,
				);
			...
			if (
				runOptions.ports.find(
					(port) => port.containerPort === inspectPort,
				) === undefined
			) {
			...
			if (
				runOptions.portsPublishAll === undefined &&
				inspectPortPublished &&
				runOptions.ports.length === 1
			) {
	...
	public static inferPackagePath(
	...
	public static getNodeOptionsForScaffolding(
		...
		if (
			packagePath &&
			path.dirname(packagePath).toLowerCase() !==
