...
export class ActivityMeasurementService implements IActivityMeasurementService {
	...
	public constructor(
		...
		private readonly requireTelemetryEnabled = true,
	) {}
					...
					new AsyncLazy(async () => {
						...
						const newValue: ActivityMeasurement = {
							...
							currentMonthSessions:
								currentValue.currentMonthSessions + 1,
	...
	public getActivityMeasurement(type: ActivityType): ActivityMeasurement {
		...
		if (!this.values.has(type)) {
			const currentValue = this.memento.get<ActivityMeasurement>(
				`vscode-docker.activity.${type}`,
				defaultMeasurement,
			);
			const now = Date.now();
...
function sameDate(
	...
	return precision === "day"
		? sameMonth && a2.getDate() === b2.getDate()
		: sameMonth;
}
