...
export function registerListeners(): void {
	if (vscode.env.isTelemetryEnabled) {
		registerEvent(
			...
			handleDocEvent("dockerfile", (context, doc) => {
				context.telemetry.properties.lineCount =
					doc.lineCount.toString();
		...
		registerEvent(
			"composefilesave",
			vscode.workspace.onDidSaveTextDocument,
			handleDocEvent("dockercompose", (context, doc) => {
				context.telemetry.properties.lineCount =
					doc.lineCount.toString();
	...
	registerEvent(
		...
		handleDocEvent("dockerfile"),
	);

	registerEvent(
		"composefileopen",
		vscode.workspace.onDidOpenTextDocument,
		handleDocEvent("dockercompose"),
	);
}

function handleDocEvent(
	languageId: string,
	additionalHandler?: docHandler,
): docHandler {
