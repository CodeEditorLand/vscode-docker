...
export async function reconnectRegistry(
	context: IActionContext,
	node?: UnifiedRegistryItem<RegistryConnectError>,
): Promise<void> {
	if (!node?.provider || !node?.wrappedItem || !node?.parent) {
		...
		throw new Error(
			l10n.t(
				"Unable to determine provider to re-enter credentials. Please disconnect and connect again.",
			),
		);
	}
