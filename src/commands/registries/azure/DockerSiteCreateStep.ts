...
export class DockerSiteCreateStep extends AzureWizardExecuteStep<IAppServiceContainerWizardContext> {
	...
	public constructor(
		private readonly tagItem: UnifiedRegistryItem<CommonTag>,
	) {
		super();
	}
	...
	public async execute(
		...
		const creatingNewApp: string = l10n.t(
			'Creating web app "{0}"...',
			context.newSiteName,
		);
	...
	private async getNewSiteConfig(
		...
		const registryTI: UnifiedRegistryItem<CommonRegistry> = this.tagItem
		...
		if (
			isAzureRegistry(registryTI.wrappedItem) &&
			!context.customLocation
		) {
		...
		// ACR -> Arc App Service. Use regular auth. Same as any V2 registry but different way of getting auth.
			...
			if (!cred?.username || !cred?.passwords?.[0]?.value) {
				throw new Error(
					l10n.t(
						"Azure App service deployment on Azure Arc only supports running images from Azure Container Registries with admin enabled",
					),
				);
			}
		...
		// Other registries -> App Service *OR* Arc App Service
			...
			if (!registryTI.provider.getLoginInformation) {
				throw new Error(
					l10n.t(
						"This registry does not support deploying to Azure App Service",
					),
				);
			}
		...
		if (context.webSitesPort) {
			appSettings.push({
				name: "WEBSITES_PORT",
				value: context.webSitesPort.toString(),
			});
		}
