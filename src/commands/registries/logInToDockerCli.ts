...
export async function logInToDockerCli(
	...
	node?: UnifiedRegistryItem<CommonRegistry>,
): Promise<void> {
	if (!node) {
		node = await registryExperience<CommonRegistry>(context, {
	...
	if (!username || !secret) {
		ext.outputChannel.warn(
			vscode.l10n.t(
				'Skipping login for "{0}" because it does not require authentication.',
				node.provider.label,
			),
		);
	} else {
		...
		await vscode.window.withProgress(progressOptions, async () => {
			try {
				...
				if (
					/error storing credentials.*The stub received bad data/i.test(
						error.message,
					)
				) {
