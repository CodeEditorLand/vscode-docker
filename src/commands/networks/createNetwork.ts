...
export async function createNetwork(context: IActionContext): Promise<void> {
	...
	const drivers: { label: string }[] =
		osType === "windows"
			? [{ label: "nat" }, { label: "transparent" }]
			: [{ label: "bridge" }, { label: "host" }, { label: "macvlan" }];

	const driverSelection = await context.ui.showQuickPick(drivers, {
		canPickMany: false,
		placeHolder: l10n.t(
			'Select the network driver to use (default is "bridge").',
		),
	});
