...
export class TaskCommandRunnerFactory implements ICommandRunnerFactory {
	...
	public getCommandRunner(): CommandRunner {
		return async <T>(
			commandResponseLike: Like<
				PromiseCommandResponse<T> | VoidCommandResponse
			>,
		) => {
	...
	public getStreamingCommandRunner(): StreamingCommandRunner {
		throw new CommandNotSupportedError(
			"Streaming commands are not supported for task runners",
		);
	}
}

async function executeAsTask(
	options: TaskCommandRunnerOptions,
	command: string,
	args?: CommandLineArgs,
): Promise<void> {
	const shellExecutionOptions = {
		cwd:
			options.cwd || options.workspaceFolder?.uri?.fsPath || os.homedir(),
	};
