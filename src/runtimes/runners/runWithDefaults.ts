...
type ClientCallback<TClient, T> = (
	client: TClient,
) => Like<PromiseCommandResponse<T>>;
type VoidClientCallback<TClient> = (
	client: TClient,
) => Like<VoidCommandResponse>;
type StreamingClientCallback<TClient, T> = (
	client: TClient,
) => Like<GeneratorCommandResponse<T>>;
...
type DefaultEnvStreamCommandRunnerOptions = Omit<
	ShellStreamCommandRunnerOptions,
	"env" | "shell" | "shellProvider" | "stdErrPipe" | "strict"
>;
...
export async function runWithDefaults<T>(
	callback:
		...
		| VoidClientCallback<IContainersClient>,
	additionalOptions?: DefaultEnvStreamCommandRunnerOptions,
): Promise<T | void> {
...
export function streamWithDefaults<T>(
	callback: StreamingClientCallback<IContainersClient, T>,
	additionalOptions?: DefaultEnvStreamCommandRunnerOptions,
): AsyncGenerator<T> {
	...
	try {
		...
		if (response.parse) {
			return await factory.getCommandRunner()(
				response as PromiseCommandResponse<T>,
			);
		} else {
...
class DefaultEnvStreamCommandRunnerFactory<
		TOptions extends DefaultEnvStreamCommandRunnerOptions,
	>
	extends ShellStreamCommandRunnerFactory<ShellStreamCommandRunnerOptions>
