...
export function isLogLevelEnabled(
	...
	return (
		outputChannel &&
		outputChannel.logLevel > 0 &&
		outputChannel.logLevel <= logLevel
	);
}

export function logProcessEnvironment(
	outputChannel: vscode.LogOutputChannel,
): void {
	if (isLogLevelEnabled(outputChannel, vscode.LogLevel.Debug)) {
		try {
			outputChannel.debug(
				`--- Process Environment (${Object.getOwnPropertyNames(process.env).length}) ---`,
			);
...
export function logDockerEnvironment(
	outputChannel: vscode.LogOutputChannel,
): void {
	if (isLogLevelEnabled(outputChannel, vscode.LogLevel.Debug)) {
		try {
