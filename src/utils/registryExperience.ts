...
import { commands, l10n, MessageItem, TreeItem, window } from "vscode";
...
import {
	UnifiedRegistryItem,
	UnifiedRegistryTreeDataProvider,
} from "../tree/registries/UnifiedRegistryTreeDataProvider";
...
export interface RegistryExperienceOptions
	extends Partial<ContextValueFilterQuickPickOptions> {
	// if registryFilter is undefined, all registries will be shown in the quick pick
	registryFilter?: RegistryFilter;
}

export async function registryExperience<TNode extends CommonRegistryItem>(
	context: IActionContext,
	options?: RegistryExperienceOptions,
): Promise<UnifiedRegistryItem<TNode>> {
	...
	if (
		...
		void window
			.showErrorMessage(
				l10n.t(
					"No registry providers are connected. Please connect a registry provider and try again to continue.",
				),
				...[add],
			)
			.then((result) => {
	...
	const unifiedRegistryItem = await runQuickPickWizard<
		UnifiedRegistryItem<TNode>
	>(context, {
		hideStepCount: true,
		promptSteps: [
...
export async function subscriptionExperience(
	context: IActionContext,
): Promise<UnifiedRegistryItem<AzureSubscriptionRegistryItem>> {
...
export class RegistryProviderQuickPickStep extends GenericQuickPickStep<
	QuickPickWizardContext,
...
> {
	...
	public async getSubWizard(
		wizardContext: QuickPickWizardContext,
	): Promise<IWizardOptions<QuickPickWizardContext>> {
		if (this.pickOptions.contextValueFilter) {
			return {
				promptSteps: [
					new RecursiveQuickPickStep(
						this.treeDataProvider,
						this.pickOptions as ContextValueFilterQuickPickOptions,
					),
				],
				hideStepCount: true,
			};
