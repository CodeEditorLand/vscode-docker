...
export class NetworksTreeItem extends LocalRootTreeItemBase<
	ListNetworkItem,
	NetworkProperty
> {
	...
	public async getItems(context: IActionContext): Promise<ListNetworkItem[]> {
		...
		let networks =
			(await ext.runWithDefaults((client) => client.listNetworks({}))) ||
			[];

		if (!showBuiltInNetworks) {
			networks = networks.filter(
				(network) => !builtInNetworks.includes(network.name),
			);
		}
