...
export class OutdatedImageChecker {
	...
	public markOutdatedImages(images: DatedDockerImage[]): void {
			...
			void callWithTelemetryAndErrorHandling(
				...
				async (context: IActionContext) => {
					...
					for (const image of images) {
						...
						const matchingRegistry = registries.find((r) =>
							r.isMatch(image.image),
						);
					...
					context.telemetry.measurements.imagesChecked =
						imageCheckPromises.length;

					// Load the data for all images then force the tree to refresh
					await Promise.all(imageCheckPromises);

					context.telemetry.measurements.outdatedImages =
						this.outdatedImageIds.length;
		...
		for (const image of images) {
			image.outdated = this.outdatedImageIds.some(
				(i) => i.toLowerCase() === image.id.toLowerCase(),
			);
		}
	}
	...
	private async checkImage(
		...
		image: ListImagesItem,
	): Promise<"latest" | "outdated" | "unknown"> {
		try {
			...
			const latestImageDigest = await this.getLatestImageDigest(
				registry,
				imageNameInfo.image,
				imageNameInfo.tag,
			);
			...
			if (
				imageInspectInfo?.repoDigests?.every(
					(digest) =>
						digest
							?.toLowerCase()
							?.indexOf(latestImageDigest.toLowerCase()) < 0,
				)
			) {
				return "outdated";
			}
			...
			// Errors are expected, e.g. all untagged local images are treated as if they are in docker.io/library, but will 404 when queried
			return "unknown";
		}
	}

	private async getLatestImageDigest(
		registry: ImageRegistry,
		repo: string,
		tag: string,
	): Promise<string> {
		const manifestResponse = await httpRequest(
			`${registry.baseUrl}/${repo}/manifests/${tag}`,
			this.defaultRequestOptions,
			async (request) => {
				if (registry.signRequest) {
					return registry.signRequest(
						request,
						`repository:library/${repo}:pull`,
					);
				}
